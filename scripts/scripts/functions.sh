...
# echo the title.
echo "[scripts] Create /scripts/functions ...";

# The contents of /scripts/functions file.
cat > $INITRD_DIR/scripts/functions << "EOF"
# -*- shell-script -*-
...
panic()
{
	...
	modprobe atkbd
	echo $@
	PS1='(initramfs):$PWD# ' /bin/sh -i </dev/console >/dev/console 2>&1
}
...
chmod 644 $INITRD_DIR/scripts/functions;
chown 0:0 $INITRD_DIR/scripts/functions;
